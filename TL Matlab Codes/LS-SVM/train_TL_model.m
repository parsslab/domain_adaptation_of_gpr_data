...
classes{3} = names{3};  % source class for multi model
classes{4} = names{1};  % source class for multi model
...
beta_mean = mean(beta_rpt);
Std_Dev_beta = std(beta_rpt);
...
set(gca,'YGrid','on');
set(gca,'XGrid','on');
set(gca,'XTick',0:2:trCnt, 'FontSize', 16, 'fontweight','b')
...
h = legend('BA: Yok', 'BA: Çoklu','BA: Tekli',3);
